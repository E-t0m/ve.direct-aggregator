// VE.Direct readtext_sendhex — Teensy 4.1
...
//       For readtext_sendhex TX pins also need level shifter (3.3V → 5V)
//
// ── Output format ───────────────────────────────────────────────────────
...
char  known_pid[7][16];                          // PID of directly connected device
bool  pid_known[7]  = {false, false, false, false, false, false, false};
unsigned long pid_last_seen[7] = {0, 0, 0, 0, 0, 0, 0};  // millis() of last direct PID
float known_vbat[7] = {VBAT_FALLBACK, VBAT_FALLBACK, VBAT_FALLBACK, VBAT_FALLBACK, VBAT_FALLBACK, VBAT_FALLBACK, VBAT_FALLBACK};

// route table: PIDs seen arriving from each port (direct + forwarded blocks)
char  route_pid[7][MAX_ROUTES][16];              // PIDs seen on each port
unsigned long route_last_seen[7][MAX_ROUTES];    // millis() of last block
int   route_cnt[7] = {0, 0, 0, 0, 0, 0, 0};  // number of known routes per port

// --- command buffer ---
char cmd_buf[CMD_BUF_SIZE];
int  cmd_len = 0;
bool hex_busy[7] = {false, false, false, false, false, false, false};   // per-port HEX busy flag
...
void exec_hex(int idx, const char* hex_str) {
	...
	if (!wait_hex_reply(port, reply, sizeof(reply))) {
		char msg[48]; sprintf(msg, "ERR %s timeout\n", pid);
		hex_busy[idx] = false; SEROUT.print(msg); return;
	}
	char msg[96];
