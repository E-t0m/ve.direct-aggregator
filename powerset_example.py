...
# of the readtext_sendhex firmware.
#
...
class PowersetClient:
	"""
	Blocking interface to the readtext_sendhex firmware.
	...
	def set_watts(self, pid, watts):
		"""
		Limit charge power of one device or all.
	...
	def hex_cmd(self, pid, hex_str):
		"""
		Send arbitrary VE.Direct HEX string to one device or all.
	...
	def restore_text_mode(self, pid='ALL'):
		"""Switch device(s) back to VE.Direct text mode."""
	...
	def _run(self, port, baud):
		...
		while not self._stop.is_set():
			...
			try:
				...
				print(f'[readtext_sendhex] open error: {e}')
				sleep(2)
				continue
			...
			try:
				...
				print(f'[readtext_sendhex] send error: {e}')
				try: ser.close()
